...
1) Auth migration phase 1: Better Auth setup, Prisma schema draft, env plan.
2) Profiles: design UserProfile model + profile edit UI plan.
3) Optional: OAuth providers (Google/Apple) via Better Auth.
4) Optional: public profile pages with privacy controls.
5) Pre-deploy: manual QA pass for Reviews, Lists, To Watch, Review detail.
6) Lint cleanup: resolve remaining lint errors/warnings.
7) Apply typography tokens across pages (headings, body, and secondary text).
8) Review entry flow: confirm dropdown alignment and close behavior after selection.
9) Add automated smoke tests for critical endpoints (run in CI or on deploy).
10) Add error monitoring/alerts (Sentry or Vercel monitoring).
